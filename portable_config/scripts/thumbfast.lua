...
local function thumb(time, r_x, r_y, script)
    ...
    -- Get display scaling factor (defaults to 1 if not available)
    local scale = properties["display-hidpi-scale"] or 1
    ...
    if r_x == "" or r_y == "" then
        ...
        x = math.floor(r_x * scale + 0.5)
        y = math.floor(r_y * scale + 0.5)

        -- Center the thumbnail horizontally above the mouse pointer
        local thumb_width = real_w or effective_w or 200  -- Use real_w if available, otherwise effective_w or default
        x = x - math.floor(thumb_width / 2 + 0.5)

        -- Adjust y to position slightly above the seekbar (add a small offset)
        -- This ensures it's always above the bottom, even with scaling/UI variations
        local offset_above_seekbar = 10  -- Adjust this value (in physical pixels) as needed for your setup
        y = y - offset_above_seekbar
    end
